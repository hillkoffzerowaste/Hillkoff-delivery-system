...
function PackSalesOrderDetails({ order }) {
  const fields = [
    ...
    ["ผู้เปิดออเดอร์", order.orderEntrySource === "pack_assist" ? `ห้องแพ็คช่วยคีย์: ${order.createdByName || order.salesName || "ห้องแพ็ค"}` : order.orderEntrySource === "store_assist" ? `สโตร์ช่วยคีย์: ${order.createdByName || order.salesName || "สโตร์"}` : `ฝ่ายขาย: ${order.createdByName || order.salesName || "-"}`],
...
function OrderCreatedAt({ order }) {
  ...
  const source = order.orderEntrySource === "pack_assist" ? "ห้องแพ็คช่วยคีย์" : order.orderEntrySource === "store_assist" ? "สโตร์ช่วยคีย์" : "ฝ่ายขายเปิดออเดอร์";
...
export default function App() {
  ...
  const [storefrontSearchDraft, setStorefrontSearchDraft] = useState("");
  const [storefrontStatusFilter, setStorefrontStatusFilter] = useState("all");
  const [storefrontDateFilter, setStorefrontDateFilter] = useState("");
  const [storefrontShowHistory, setStorefrontShowHistory] = useState(false);
  const [storefrontSelectedIds, setStorefrontSelectedIds] = useState([]);
  const [storefrontBatchPending, setStorefrontBatchPending] = useState(false);
  ...
  const buildKpiActivityRows = (sourceOrders, department) => sourceOrders.flatMap(order => {
    ...
    return events.filter(Boolean).map((item, index) => {
      ...
      else if (item.action === "store_booking_update") title = item.result === "booking_numbers_confirmed" ? "สโตร์ยืนยันเลขใบสั่งจอง" : "สโตร์บันทึกร่างเลขใบสั่งจอง";
  ...
  useEffect(() => {
    ...
    if (shouldFetchReports) fetchStoreReports({ date: (isStoreBookingEntryView || ["store-booking", "store-online", "pack-booking", "pack-online"].includes(displayTab)) && !storeReportSearchActive ? storeReportDate : "", query: storeReportSearchActive ? storeReportQuery : "", type: reportType, includeDeleted: isKpi || storeReportIncludeDeleted, kpi: isKpi });
    const isStoreReport = auth.role === "store" && (isStoreBookingEntryView || ["store-booking", "store-online"].includes(displayTab));
    ...
    return () => window.clearInterval(timer);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [auth.role, displayTab, storeWorkSubtab, storeReportDate, storeReportSearchActive, storeReportIncludeDeleted, kpiAutoRefresh]);
  ...
  useEffect(() => {
    if (!["sales", "admin", "store", "pack"].includes(auth.role)) return;
  ...
  const createOrder = async () => {
    ...
    const preparation = initialPreparationStatuses(orderForm.deliveryMethod, orderForm.workflowType);
  ...
  const closeUrgentOrderModal = () => {
    ...
    const preparation = initialPreparationStatuses(pendingOrder.deliveryMethod, pendingOrder.workflowType);
    const orderToCreate = { ...pendingOrder, ...preparation, shippingCarrier: pendingOrder.deliveryMethod === "outstation" ? resolvedShippingCarrier : "" };
  ...
  const completeStorefrontSelected = async (ids, action = "grab_pickup") => {
    ...
    setStorefrontSelectedIds([]);
    setStorefrontBatchPending(false);
    setSyncStatus(failed.length ? `⚠️ ปิดงานสำเร็จ ${orderIds.length - failed.length} รายการ · ข้าม ${failed.length} รายการ` : `✅ ปิดงานสำเร็จ ${orderIds.length} รายการ`);
  };
      ...
      <section className="workspace">
        ...
        {displayTab === "storefront-pickup" && (
          <section className="panel role-workspace ops-workspace">
            {(() => {
              const date = storefrontDateFilter || todayServiceDate;
              const visibleOrders = filterStorefrontOrders(orders, { date: storefrontShowHistory ? storefrontDateFilter : date, query: storefrontSearch, status: storefrontStatusFilter, history: storefrontShowHistory, today: todayServiceDate });
              const selectableIds = storefrontSelectableOrderIds(visibleOrders);
              const selectedVisibleIds = storefrontSelectedIds.filter((id) => selectableIds.includes(id));
              const allSelected = selectableIds.length > 0 && selectableIds.every((id) => selectedVisibleIds.includes(id));
              return <>
            <div className="panel-head"><h2><Store size={17} className="i-inline" aria-hidden="true" /> Grab / รับหน้าร้าน</h2><span>{visibleOrders.length} งาน{storefrontShowHistory ? "ในประวัติ" : "วันนี้"}</span></div>
            <p className="muted" style={{ marginTop: 0 }}>หน้าหลักแสดงเฉพาะออเดอร์วันนี้ · ออเดอร์เก็บไว้ในประวัติ ค้นหาและกรองสถานะได้</p>
            <div className="store-report-filters" style={{ marginBottom: "var(--sp-5)" }}>
              <input aria-label="กรองวันที่หน้าร้าน" type="date" value={storefrontShowHistory ? (storefrontDateFilter || "") : todayServiceDate} max={todayServiceDate} disabled={!storefrontShowHistory} onChange={(event) => setStorefrontDateFilter(event.target.value)} />
            ...
            <div style={{ display: "grid", gap: "var(--sp-5)" }}>
              {visibleOrders.length === 0 ? <p className="muted">{storefrontShowHistory ? "ยังไม่พบออเดอร์ในประวัติที่ตรงกับตัวกรอง" : "ไม่มีออเดอร์วันนี้ในขณะนี้"}</p> : visibleOrders.map((order) => {
                ...
                return <article key={order.id} className="role-order-card" style={{ display: "grid", gap: "var(--sp-4)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: "var(--sp-5)", flexWrap: "wrap" }}>
                    {!storefrontShowHistory && <input type="checkbox" aria-label={`เลือก ${order.id}`} checked={selectedVisibleIds.includes(order.id)} disabled={!ready || handedOver} onChange={(event) => setStorefrontSelectedIds((current) => event.target.checked ? [...new Set([...current, order.id])] : current.filter((id) => id !== order.id))} />}
        ...
        {["pack-work", "pack-pickup", "pack-outstation"].includes(displayTab) && (
          <section className="panel role-workspace ops-workspace">
            ...
            {displayTab === "pack-work" && <section aria-label="คิวคนขับ" style={{ marginBottom: "var(--sp-6)", display: "grid", gap: "var(--sp-4)", background: "var(--c-brand-bg)", border: "1px solid var(--c-brand-border)", borderLeft: "5px solid var(--c-brand)", borderRadius: "10px", padding: "var(--sp-5)" }}><div className="panel-head" style={{ margin: 0 }}><h3 style={{ margin: 0, color: "var(--c-brand-dark)" }}><Car size={16} className="i-inline" aria-hidden="true" /> คิวคนขับแล้ว</h3><span>{packDriverQueueOrders.length} งาน</span></div>{packDriverQueueOrders.length > 0 ? <div style={{ display: "grid", gap: "var(--sp-3)" }}>{packDriverQueueOrders.map(order => <article key={order.id} style={{ background: "var(--c-surface)", border: "1px solid var(--c-brand-border)", borderRadius: "7px", padding: "var(--sp-4)", display: "grid", gap: "var(--sp-2)" }}><div style={{ display: "flex", justifyContent: "space-between", gap: "var(--sp-4)", flexWrap: "wrap" }}><div><b>{order.id} · {order.customerName || "-"}</b><div className="muted">{[order.zone, order.address].filter(Boolean).join(" · ") || "-"}</div></div><span className="status-chip" style={{ color: "var(--c-brand-dark)", background: "var(--c-brand-bg-strong)" }}>{order.driverId ? `คนขับรับแล้ว · ${order.driverName || order.driverId}` : "รอคนขับรับ"}</span></div><small className="muted">ห้องแพ็ค: {WORKFLOW_STATUS_META[order.packStatus]?.label || order.packStatus || "-"} · สถานะส่ง: {order.status || "-"} · เข้าคิว {formatThaiDateTime(order.queuedAt || order.updatedAt || order.createdAt)}</small></article>)}</div> : <p className="muted" style={{ margin: 0 }}>ยังไม่มีออเดอร์ที่เข้าคิวคนขับ</p>}</section>}
          ...
          <div style={{ position: "fixed", inset: 0, background: "var(--c-overlay)", zIndex: 1500, display: "grid", placeItems: "center", padding: "var(--sp-7)" }}>
            <section className="panel" style={{ width: "min(620px, 100%)", maxHeight: "90vh", overflowY: "auto" }}>
              ...
              <div style={{ display: "grid", gap: "var(--sp-5)" }}>
                ...
                {workModal.takeOverStoreCheck && <div style={{ background: "var(--c-warn-bg-strong)", border: "1px solid var(--c-warn-border)", color: "var(--c-warn-deep)", borderRadius: "8px", padding: "var(--sp-4)", fontSize: "12px", fontWeight: 700 }}>สโตร์ยังไม่ได้ส่งออเดอร์เข้าห้องแพ็ค · การยืนยันครั้งนี้จะบันทึกว่าห้องแพ็คตรวจแทนสโตร์ และถ้าตรวจครบจะเข้าคิวคนขับทันที</div>}
