...
export default function App() {
  ...
  const salesOutstationStoreOrders = (orders || []).filter(order => order.deliveryMethod === "outstation" && order.workflowType === "store_route" && (order.queueStatus !== "outstation_ready" || ["partial", "waiting"].includes(order.storeStatus)));
          ...
          <section className="panel role-workspace">
            ...
            {displayTab === "store-work" && <div style={{ display: "grid", gap: "10px" }}>
              ...
              {storeWorkOrders.map(order => { const storePending = ["partial", "waiting"].includes(order.storeStatus) || (order.missingItems || []).length > 0; return <article key={order.id} className="role-order-card" style={storePending ? { borderColor: order.storeStatus === "partial" ? "#fb923c" : "#facc15", borderLeft: `5px solid ${order.storeStatus === "partial" ? "#f97316" : "#eab308"}`, background: order.storeStatus === "partial" ? "#fff7ed" : "#fefce8" } : undefined}>
                ...
                {storePending && <b style={{ color: order.storeStatus === "partial" ? "#c2410c" : "#a16207", fontSize: "12px" }}>⚠️ รอของ / ของยังไม่ครบ — ติดตามและอัปเดทเมื่อของเข้า</b>}
            ...
            {["store-outstation", "store-online"].includes(displayTab) && <div style={{ display: "grid", gap: "10px" }}>
              {(() => { const type = displayTab === "store-outstation" ? "outstation" : "online"; const selectedRows = storeReports.filter(item => item.type === type && (storeReportSearchActive || String(item.createdAt || "").slice(0, 10) === storeReportDate)); const overdue = storeReports.filter(item => item.type === type && !item.confirmedAt && !item.deletedAt && ["draft", "waiting", "partial"].includes(item.status) && String(item.createdAt || "").slice(0, 10) < todayServiceDate); return <>
                {type === "outstation" && <section style={{ display: "grid", gap: "8px", border: "1px solid #bae6c8", background: "#f4fbf5", borderRadius: "10px", padding: "12px" }}><div className="panel-head"><h3>งานต่างจังหวัดจากฝ่ายขาย</h3><span>{salesOutstationStoreOrders.length} งาน</span></div>{salesOutstationStoreOrders.map(order => { const storePending = ["partial", "waiting"].includes(order.storeStatus) || (order.missingItems || []).length > 0; return <article key={order.id} className="role-order-card" style={storePending ? { borderColor: order.storeStatus === "partial" ? "#fb923c" : "#facc15", borderLeft: `5px solid ${order.storeStatus === "partial" ? "#f97316" : "#eab308"}`, background: order.storeStatus === "partial" ? "#fff7ed" : "#fefce8" } : undefined}><div style={{ display: "flex", justifyContent: "space-between", gap: "10px", flexWrap: "wrap" }}><b>{order.id} · {order.customerName}</b><span className="status-chip">สโตร์: {order.storeStatus || "pending"}</span></div><div className="muted">ใบสั่งจอง: {order.bookingNumber || "ยังไม่ระบุ"} · ขนส่ง: {order.shippingCarrier || "-"}</div>{storePending && <b style={{ color: order.storeStatus === "partial" ? "#c2410c" : "#a16207", fontSize: "12px" }}>⚠️ รอของ / ของยังไม่ครบ — ติดตามและอัปเดทเมื่อของเข้า</b>}<details className="prep-order-details"><summary>ดูรายละเอียดจากฝ่ายขาย</summary><PackSalesOrderDetails order={order} /></details><button className={storePending ? "secondary" : "primary"} onClick={() => openWorkModal(order, "store")}>{storePending ? "อัปเดทออเดอร์" : "รับงาน / บันทึกสโตร์"}</button></article>})}{!salesOutstationStoreOrders.length && <p className="muted">ยังไม่มีงานต่างจังหวัดจากฝ่ายขายที่รอดำเนินการ</p>}</section>}
