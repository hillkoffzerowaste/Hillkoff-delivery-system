...
export default function App() {
  ...
  const packWorkOrders = preparationOrders.filter(order => order.deliveryMethod !== "outstation" && order.packStatus !== "blocked" && isOpenPackQueueStatus(order.packStatus));
          ...
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.48)", zIndex: 1500, display: "grid", placeItems: "center", padding: "16px" }}>
            <section className="panel" style={{ width: "min(820px, 100%)", maxHeight: "90vh", overflowY: "auto" }}>
              ...
              <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginTop: "12px", padding: "10px", borderRadius: "8px", background: "#f8fafc", border: "1px solid #dbe4ee" }}>
                {auth.role === "pack" && chiangmaiHistoryOrder.workflowType !== "direct_driver" && chiangmaiHistoryOrder.packStatus !== "returned" && <button className="secondary danger" onClick={() => { const order = chiangmaiHistoryOrder; setChiangmaiHistoryOrder(null); openWorkModal(order, "pack"); }}>⚠️ พบปัญหา · ส่งกลับสโตร์ตรวจสอบ</button>}
