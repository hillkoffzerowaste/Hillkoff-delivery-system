        ...
        <nav className="app-nav" aria-label="เมนูหลัก">
          ...
          {auth.role === "store" && (
            <>
              ...
              <button type="button" className={displayTab === "store-pickup" ? "active" : ""} onClick={() => selectAppTab("store-pickup")}><Store size={18} /> Grab/รับหน้าร้าน{storePickupOrders.length > 0 && <span className="nav-count-badge" aria-label={`งาน Grab หรือรับหน้าร้าน ${storePickupOrders.length} งาน`}>{storePickupOrders.length}</span>}</button>
