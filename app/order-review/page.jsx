...
export default function OrderReviewPage() {
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#f0fdf4", padding: "24px 14px", display: "grid", placeItems: "start center" }}>
      <section className="panel" style={{ width: "min(520px, 100%)", marginTop: "5vh", display: "grid", gap: "16px", borderTop: "5px solid #176b3a" }}>
        <header style={{ display: "grid", gap: "4px" }}>
          <b style={{ color: "#176b3a", fontSize: "13px" }}>HILLKOFF DELIVERY</b>
        ...
        {order && (status === "ready" || status === "submitted") && (
          <form onSubmit={submitReview} style={{ display: "grid", gap: "14px" }}>
            <div style={{ background: order.deliveryCompleteness === "incomplete" ? "#fff7ed" : "#f0fdf4", borderRadius: "8px", padding: "10px", fontSize: "13px" }}>
            ...
            {message && <div role="status" style={{ color: message.startsWith("ขอบคุณ") ? "#166534" : "#991b1b", fontWeight: 700 }}>{message}</div>}
          </form>
        )}
      </section>
    </main>
