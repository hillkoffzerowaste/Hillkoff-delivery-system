...
const cardLabels = {
  ...
  routeTasks: "งานเส้นทาง", chiangmaiWaiting: "เชียงใหม่รอจัดส่ง", chiangmaiBacklog: "เชียงใหม่ค้างส่ง", outstationWaiting: "ต่างจังหวัดรอจัดส่ง"
};
...
export default function DispatchDashboard({ apiFetch, role, onDeleteOrder, onResetOrders }) {
  ...
  return <div className="dispatch-dashboard-new">
    ...
    <section className="analytics-cards dispatch-eight-cards">{Object.entries(cardLabels).map(([key, label]) => <div key={key}><span>{label}</span><b>{data.cards[key] || 0}</b></div>)}</section>
