...
export async function POST(request) {
  try {
    ...
    if (!orders.length) return Response.json({ ok: false, error: "No active orders in this round" }, { status: 404 });
    if (orders.length > 200) return Response.json({ ok: false, error: "Round exceeds 200 orders" }, { status: 409 });
    ...
    await db.runTransaction(async (transaction) => {
      ...
      const currentOrders = currentSnaps.filter((doc) => doc.exists).map((doc) => ({ id: doc.id, ref: doc.ref, ...doc.data() }));
