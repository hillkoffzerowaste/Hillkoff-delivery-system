...
export async function resolveVerifiedDriver(db, decoded) {
  ...
  if (!uid) return null;
  let snap = await db.collection("users_by_phone").where("uidLast", "==", uid).limit(1).get();
  let doc = snap.docs[0] || null;
  if (!doc) {
    snap = await db.collection("users_by_phone").where("legacyUids", "array-contains", uid).limit(1).get();
