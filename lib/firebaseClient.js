...
export async function signInWithStaffCredentials(username, password) {
  const auth = getFirebaseAuth();
  const normalized = String(username || "").trim().toLowerCase();
  if (!normalized) throw new Error("Missing username");
  return signInWithEmailAndPassword(auth, `${normalized}@staff.hillkoff.local`, String(password || ""));
}
