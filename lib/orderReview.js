export const ORDER_REVIEW_QR_PREFIX = "HKO3";
...
const ORDER_REVIEW_TOKEN_PATTERN = /^[A-Za-z0-9_-]{32,128}$/;
const ORDER_REVIEW_PAYLOAD_PATTERN = /^HKO3\|([^|/\s]{1,120})\|([A-Za-z0-9_-]{32,128})$/;
...
export function createOrderReviewPayload(orderId, token) {
  const normalized = String(orderId || "").trim();
  if (!isValidOrderReviewOrderId(normalized)) throw new Error("Invalid order ID");
  const normalizedToken = String(token || "").trim();
  if (!ORDER_REVIEW_TOKEN_PATTERN.test(normalizedToken)) throw new Error("Invalid order review token");
  return `${ORDER_REVIEW_QR_PREFIX}|${normalized}|${normalizedToken}`;
}

export function createOrderReviewUrl(origin, orderId, token) {
  const url = new URL(ORDER_REVIEW_PATH, String(origin));
  url.searchParams.set("t", createOrderReviewPayload(orderId, token));
  return url.toString();
}
