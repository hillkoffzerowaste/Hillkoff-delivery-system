...
export function isChiangmaiWaitingForDate(order, selectedDate) {
  return classifyOrderArea(order) === "city" && isOpen(order) && orderCreatedDateKey(order) === selectedDate;
}

export function isChiangmaiBacklogForDate(order, selectedDate) {
  const created = orderCreatedDateKey(order);
  return classifyOrderArea(order) === "city" && isOpen(order) && Boolean(created) && created < selectedDate;
}
...
export async function resolveDeliveryVehicleSnapshot(db, { driverId, deliveryServiceDate }) {
  const snap = await db.collection("vehicle_usage_events").where("driverId", "==", driverId).limit(200).get();
