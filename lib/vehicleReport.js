...
export function buildVehicleReport({
  ...
  const ensure = (date, vehicle, driver, source = {}) => {
    ...
    if (!rows.has(key)) rows.set(key, {
      ...
      driverName: source.driverName || "", odometerStart: null, odometerEnd: null,
      odometerStartEventId: "", odometerEndEventId: "", distanceKm: 0,
  ...
  for (const event of usageEvents) {
    ...
    if (Number.isFinite(odo)) {
      ...
      if (event.eventType === "end" && (row.odometerEnd == null || odo > row.odometerEnd)) {
        row.odometerEnd = odo;
        row.odometerEndEventId = String(event.id || "");
      }
    }
  }
