...
function getQueueDateKey(order = {}) {
  const driverQueueDate = String(order.driverQueueDate || "");
  if (DATE_PATTERN.test(driverQueueDate)) return driverQueueDate;

  const serviceDate = String(order.serviceDate || "");
  if (DATE_PATTERN.test(serviceDate)) return serviceDate;

  const timestamp = order.queuedAt || order.createdAt;
  return timestamp ? bangkokDateKey(timestamp) : "";
}

function hasExpiredQueueDate(order = {}, today) {
  const queueDate = getQueueDateKey(order);
