...
export function canPackTakeOverStoreCheck(order = {}) {
  return isStoreCheckingForPack(order)
    && order.packStatus !== "checked"
    && !order.driverId;
}

export function getPackStoreCheckingOrders(orders = []) {
  return orders.filter(canPackTakeOverStoreCheck).slice().sort((left, right) => {
