...
export function apiClientProfile(client, allowedRoles = [], requestedRole = '') {
  const roles = normalizeRoles(client.roles);
  const requested = clean(requestedRole, 40);
  const role = requested
    ? (roles.includes(requested) && (!allowedRoles.length || allowedRoles.includes(requested)) ? requested : '')
    : allowedRoles.length ? roles.find((item) => allowedRoles.includes(item)) : roles[0];
